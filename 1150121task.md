...
| Phase 1 | 工具列按鈕邊距 | 0.5 天 | 🔲 待開始 |
| Phase 2 | 病患研究瀏覽器 | 5-7 天 | ✅ 已完成 (2.1.4 待做) |
| Phase 2 | 進階回放控制 | 3-5 天 | 🔲 待開始 |
| Phase 2 | 互動體驗優化 | 3-5 天 | 🔲 待開始 |
| Phase 3 | 臨床量測工具 | 7-10 天 | 🔲 待開始 |
| Phase 3 | 專業報告匯出 | 5-7 天 | 🔲 待開始 |
| Phase 3 | 臨床工具智能化 | 10-14 天 | 🔲 待開始 |
| Phase 3 | 輸出與連結增強 | 3-5 天 | 🔲 待開始 |
...
### 1.4 工具列按鈕邊距調整 (Toolbar Button Padding)
...
- [x] **1.4.1** 調整下拉按鈕樣式
  - [x] 目標：`QToolButton[is_dropdown="true"]`
  - [x] 設定 `padding-right: 20px` (原為 45px，下修)
  - [x] 設定 `padding-left: 10px` (新增)
  - [x] 驗證箭頭區域保留 `margin-bottom: 3px`
...
**優先級：** 🔴 高  
**複雜度：** ⭐⭐⭐ 中高  
**影響檔案：** `src/qt_gui.py` (`FileListWidget`), 新增 `src/study_browser.py`

#### 任務項目

- [x] **2.1.1** 升級 `FileListWidget` 為階層式
  - [x] 改用 `QTreeView` + `QStandardItemModel`
  - [x] 設計資料結構：Patient → Study → Series

- [x] **2.1.2** DICOM 階層解析
  - [x] 從 DICOM 標籤提取 Patient/Study/Series 資訊
  - [x] 自動歸類同一病患的檔案

- [x] **2.1.3** 縮圖預覽
  - [x] 產生 Series 代表幀縮圖 (48×48)
  - [x] LRU 快取縮圖避免重複產生 (ThumbnailCache)
...
### 2.3 互動體驗優化 (Interaction Polish)

**優先級：** 🟡 中  
**複雜度：** ⭐⭐ 中等  
**影響檔案：** `src/qt_gui.py`, 新增 `src/utils/config_manager.py`
...
- [ ] **2.3.3 右鍵快顯選單 (Context Menu)**
  - [ ] 影像區域右鍵選單 (快速重設/標註/截圖)
  - [ ] 檔案列表右鍵選單 (開啟位置/加入最愛)

### 2.4 標註系統重構 (Annotation System Refactoring)

**優先級：** 🔴 高  
**複雜度：** ⭐⭐⭐ 中高  
**影響檔案：** `src/fast_annotations.py`, `src/annotations.py`, `src/qt_gui.py`, `src/viewport.py`

#### 任務項目

- [x] **2.4.1** 診斷標註消失問題
  - [x] 確認 FAST 渲染為「資料驅動」，暫停時管線停止
  - [x] 排除 LUT Overlay 覆蓋假設

- [x] **2.4.2** 改用 Qt AnnotationOverlay 渲染全部標註
  - [x] 修改 `paintEvent()` 繪製標註線條/矩形/多邊形
  - [x] 修改 `paintEvent()` 繪製測量線條
  - [x] 保留測量文字標籤渲染

- [x] **2.4.3** 座標轉換系統更新
  - [x] `CoordinateConverter` 整合 ortho_params（縮放）
  - [x] `CoordinateConverter` 整合 view_matrix translation（平移）
  - [x] 修正 Widget ↔ Image 座標轉換公式

- [x] **2.4.4** 禁用 FAST LineRenderer
  - [x] 移除 `qt_gui.py` 中所有 `fast_annotation_manager` 同步調用
  - [x] 移除 `viewport.py` 中 `ensure_renderer_added()` 調用

- [x] **2.4.5** 實作 View Matrix 監聯機制
  - [x] Timer (50ms) 定期從 FAST View 取得 view_matrix 和 ortho_params
  - [x] 變化時觸發 `AnnotationOverlay.update()` 重繪
...
### 3.3 臨床工具智能化 (Smart Clinical Tools)

**優先級：** 🟡 中  
**複雜度：** ⭐⭐⭐⭐ 高  
**影響檔案：** `src/annotations.py`, `src/image_processing.py`

#### 任務項目

- [ ] **3.3.1 精確放大鏡 (Precision Loupe)**
  - [ ] 測量時游標旁顯示局部 2x/4x 放大視窗 (Picture-in-Picture)
  - [ ] 避免手指/游標遮擋測量點
  - [ ] 顯示游標中心點的精確像素值 (Pixel Value)

- [ ] **3.3.2 智慧邊緣吸附 (Smart Edge Snapping)**
  - [ ] 實作即時梯度偵測 (Gradient Detection)
  - [ ] "Magnetic Cursor"：游標靠近組織邊緣時自動吸附
  - [ ] 支援手動開/關吸附功能 (按住 Ctrl 暫時停用)

- [ ] **3.3.3 自動輪廓描繪 (Auto-Trace)**
  - [ ] 點擊封閉區域自動描邊 (如：囊腫、心室)
  - [ ] 基於 Region Growing 或 Active Contour 演算法
  - [ ] 允許使用者手動修正自動描繪的節點

### 3.4 輸出與連結增強 (Export & Connectivity)
...
- [ ] **3.4.1 影像去識別化 (Anonymization)**
  - [ ] 匯出時移除敏感 DICOM Tags (Name, ID, DOB)
  - [ ] 在保留影像品質下清除燒錄在像素上的個資 (Pixel De-identification)

- [ ] **3.4.2 動態影像匯出**
  - [ ] 將 Cine Loop 匯出為 MP4/AVI (支援 H.264)
  - [ ] 匯出為 GIF 動圖 (適合教學展示)
  - [ ] 包含/不包含標註層的選項

- [ ] **3.4.3 簡易 PACS 連線**
  - [ ] C-ECHO (連線測試)
  - [ ] C-STORE (將標註後的影像回傳至醫院 Server)
...
| 2026-01-21 | 完成 2.1 病患研究瀏覽器 (2.1.1-2.1.3) ✅ 階層顯示+縮圖正常 |
| 2026-01-22 | 新增 Phase 2.3 (互動), 3.3 (智慧工具), 3.4 (匯出) 任務規劃 |
